...
export default function FoundReservation() {
  ...
  const guestDisplay = (reservation.userId && typeof reservation.userId === 'object')
      ...
      <div className="inner-grid">
        ...
        <div className={styles.mainContentGrid}>
          <section className={styles.roomSection}>
            <div className={styles.roomHeader}>Room 1</div>
            <div className={styles.roomBody}>
              <h3>{reservation.hotelId?.name || "Fairmont Resort"}</h3>
              <ul className={styles.detailsList}>
                <li><span>Adults</span> <strong>{reservation.adults || 1}</strong></li>
                <li><span>Children</span> <strong>{reservation.children || 0}</strong></li>
          ...
          <section className={styles.priceSection}>
            ...
            <div className={styles.finalTotalTable}>
               <div>Room Price: <span>${reservation.roomPrice || reservation.totalAmount}</span></div>
               <div>Tax: <span>${reservation.tax || 0}</span></div>
               <div>Fees: <span>${reservation.fees || 0}</span></div>
               <hr />
               <div className={styles.grandTotal}>Total: <span>${reservation.totalAmount}</span></div>
            </div>
          </section>
        </div>
